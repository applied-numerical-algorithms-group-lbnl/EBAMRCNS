                                                                           ...
                                                                           const bool & a_onlyBiggest)
{
  ...
  for (int ilev = 0; ilev < m_nlevels; ilev++)
  {
    ...
    if (ilev == m_nlevels-1)
    {
      coarComponentInfo.define(curGrids,4,ghostCells,curFactory);
    }
  }
  ...
  Vector<IndexTM<Real,4> > compClasses;
  ...
  for (DataIterator dit = coarGrids.dataIterator(); dit.ok(); ++dit)
  {
    ...
    curEBCellFABInfo.setVal(0,procID());
    ...
    for (VoFIterator vofit(curIVS,curEBGraph); vofit.ok(); ++vofit)
    {
      ...
      {
        pout() << "        totalVolFrac: " << totalVolFrac << endl;
        ...
        if (totalVolFrac > 0.0)
        {
          ...
          IndexTM<Real,4> cur(procID(),boxIndex,curNum,totalVolFrac);
    ...
    for (VoFIterator vofit(curIVS,curEBGraph); vofit.ok(); ++vofit)
    {
      ...
      if (comp >= 0)
      {
        curEBCellFABInfo(curVoF,3) = compClasses[curBoxClassIndex+comp][3];
      }
    }
  ...
  for (DataIterator dit = coarGrids.dataIterator(); dit.ok(); ++dit)
  {
    CH_TIME("EBIndexSpace::connectedComponents-loop2.5");
        ...
        for (VoFIterator vofit(ivs,curEBGraph); vofit.ok(); ++vofit)
        {
          ...
          for (int iface = 0; iface < numFaces; iface++)
          {
            ...
            if (!face.isBoundary())
            {
              ...
              pout() << "--- CurVoF     : " << curEBCellFABInfo(curVoF     ,0) << ","
                                            << curEBCellFABInfo(curVoF     ,1) << ","
                                            << curEBCellFABInfo(curVoF     ,2) << ","
                                            << curEBCellFABInfo(curVoF     ,3) << endl;
              pout() << "+++ NeighborVoF: " << curEBCellFABInfo(neighborVoF,0) << ","
                                            << curEBCellFABInfo(neighborVoF,1) << ","
                                            << curEBCellFABInfo(neighborVoF,2) << ","
                                            << curEBCellFABInfo(neighborVoF,3) << endl;
              pout() << endl;
            }
          }
        }
