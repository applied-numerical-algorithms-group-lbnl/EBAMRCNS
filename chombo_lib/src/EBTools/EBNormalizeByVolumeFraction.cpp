  ...
  for(DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
    {
      ...
      for(int ivec = 0; ivec < vofvec.size(); ivec++)
        {
          ...
          getLocalStencil(localStencil, vofvec[ivec], dit());
      ...
      m_stencil[dit()] = RefCountedPtr<AggStencil<EBCellFAB, EBCellFAB > >
        (new AggStencil<EBCellFAB, EBCellFAB >(dstVoF, stencil, a_Q[dit()], a_Q[dit()]));
           ...
           const Interval& a_compInterval)
{
  ...
  for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
    {
      m_stencil[dit()]->apply(a_Q[dit()], save[dit()], begin, begin, length, false);
    }
}
//----------------------------------------------------------------------------
