...
struct BFMImpl
{
  static void f(T1* t1, const T2* t2, int* lo1, int* lo2, int* stride1, int* stride2, int* counting,F f)
  {
...
struct BFMImpl<0,T1,T2,F>
{
  static void f(T1* t1, const T2* t2, int* lo1, int* lo2, int* stride1, int* stride2, int* counting,F f)
  {
    t1+=lo1[0];
    t2+=lo2[0];
    for(int i=0; i<counting[0]; ++i, ++t1, ++t2) f(*t1,*t2);
  }
};
...
template<unsigned char D, typename T1, typename T2, typename F>
void BFM(T1* t1, const T2* t2, const Box& b1, const Box& b2, int c1, int c2, int ncomp, const Box& r, F f)
{
  ...
  if(box() == a_x.box())
    {
      ...
      for(size_t i=0; i<m_nvar*m_numpts; i++)
        m_dptr[i]*=p[i];
    }
  else
    {
...
inline void bFuncCR(Complex& a, const Real& b){a*=b;} // we can replace this with a lambda in C++11 (bvs, Dec 2015)
                             ...
                             const Box&           a_box)
{
  ...
  BFM<CH_SPACEDIM>(dataPtr(), a_x.dataPtr(),m_domain, a_x.box(), 0, 0, m_nvar, a_box, bFuncCR);
